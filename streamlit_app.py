...
if df is not None:
    ...
    if not filtered_df.empty:
        ...
        with col1: st.plotly_chart(create_gauge(total_revenue, "Revenue", rev_ranges, "$"), use_container_width=True)
        with col2: st.plotly_chart(create_gauge(food_pct, "Food Cost %", food_ranges, "%"), use_container_width=True)
        with col3: st.plotly_chart(create_gauge(labor_pct, "Labor Cost %", labor_ranges, "%"), use_container_width=True)

        st.divider()
        _, profit_col, _ = st.columns([1, 2, 1])
        with profit_col: st.plotly_chart(create_gauge(profit, "Total Profit", profit_ranges, "$"), use_container_width=True)
